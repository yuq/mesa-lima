...
#
# usage: perl scripts/get_reviewer.pl [OPTIONS] <patch>
#        perl scripts/get_reviewer.pl [OPTIONS] -f <file>
#
...
if (!top_of_mesa_tree($lk_path)) {
    die "$P: The current directory does not appear to be "
	. "a mesa source tree.\n";
}

## Read REVIEWERS for type/value pairs

my @typevalue = ();
my %keyword_hash;

open (my $maint, '<', "${lk_path}REVIEWERS")
    or die "$P: Can't open REVIEWERS: $!\n";
while (<$maint>) {
    my $line = $_;
...
foreach my $file (@ARGV) {
	...
	push(@files, $file);
	if ($file ne "REVIEWERS" && -f $file && ($keywords || $file_emails)) {
	    open(my $f, '<', $file)
...
REVIEWER field selection options:
  --email => print email address(es) if any
    --git => include recent git \*-by: signers
    --git-all-signature-types => include signers regardless of signature type
        or use only ${signature_pattern} signers (default: $email_git_all_signature_types)
    --git-fallback => use git when no exact REVIEWERS pattern (default: $email_git_fallback)
